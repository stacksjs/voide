...
` }
  ...
  // STX Utilities
  stx.useRefs = function() {
    var refs = {}
    // Check for @ref attribute (STX syntax)
    var elements = document.querySelectorAll('[\\@ref]')
    elements.forEach(function(el) {
      var name = el.getAttribute('@ref')
      if (name) {
        refs[name] = el
      }
    })
    // Also check for data-ref attribute (alternative/compiled syntax)
    var dataRefElements = document.querySelectorAll('[data-ref]')
    dataRefElements.forEach(function(el) {
      var name = el.getAttribute('data-ref')
      if (name && !refs[name]) {
        refs[name] = el
      }
    })
    // Also check for ref attribute (legacy syntax)
    var refElements = document.querySelectorAll('[ref]')
  ...
  stx.onKey = function(key, callback) {
  ...
  stx.el = function(tag, props, children) {
  ...
  stx.escapeHtml = function(text) {
    var div = document.createElement('div')
    div.textContent = text
    return div.innerHTML
  }

  stx.activeElement = function() {
    return document.activeElement
  }

  window.stx = stx
  // Also expose as STX (uppercase) for compatibility
  window.STX = stx
